...
function readLambdaName(appName: string): string | null {
  let data;
  try {
    console.log(`Read lambda name from .serverless/Template.${appName}.defaultEdgeLambda.json`)
...
function deploy(filename: string): void {
  try {
    ...
    const serverless: any = yaml.load(fs.readFileSync("serverless.yml", "utf8"))
    ...
    execSync(`aws s3 mb s3://${readBucketName()} --region us-east-1 || true`, {
      stdio: "inherit",
    });
