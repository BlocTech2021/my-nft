...
export function AssetEditTab({ asset, onAssetEdit, onAssetRemoved }: AssetEditTabProps) {
  ...
  return (
    <div className="mt-3 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
      ...
      <div className={classNames(asset.photoFrameUrl ? 'opacity-75 cursor-not-allowed' : '', "sm:col-span-6")}>
          ...
          <div className="rounded-md shadow-xs px-0 py-0 flex w-full">
            <Range
                ...
                renderThumb={({ props }) => (
                  <div
                    {...props}
                    className={classNames(asset.photoFrameUrl ? 'cursor-not-allowed' : '',
                      "w-3 h-3 transform translate-x-10 bg-indigo-500 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500")}
                  />
                )}
              />
          </div>
